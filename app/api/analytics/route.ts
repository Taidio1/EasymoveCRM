...
const generateMockAnalyticsData = () => {
  ...
  for (let i = 29; i >= 0; i--) {
    ...
    dailyMetrics.push({
      ...
      conversions: Math.round(baseSessions * randomFactor * 0.05),
    })
  }
...
export async function GET(request: NextRequest) {
  try {
    ...
    // Fallback: brak skonfigurowanych credentiali GA4
    return NextResponse.json({
      success: true,
    ...
    return NextResponse.json(
      { success: false, error: 'Nie udało się pobrać danych analitycznych' },
      { status: 500 }
    )
  }
}
